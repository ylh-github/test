...
let caigouyuan = []
let caigy = [];
let url = Cake.Piper.getAuthUrl('../../tdhc_cgsys/ch/searchTpChorderst');
let caigouzhuangtai = []
let fenpeizhuangtai = []
let wuzileixing = []
let shifoujiju = []
      ...
      /*  scrolling: {
        ...
        columns: [
            ...
            },
            {
                ...
                caption: '采购员',
                calculateDisplayValue: function (rowData) {
                    let matchedItem = caigy.find(item => item.cSubitemid == rowData.cManor);
                    if (matchedItem == null || matchedItem == undefined) {
                        return "";
                    } else {
                        return matchedItem.cSubitemdes;
...
//        $.ajax({
//            url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_q002mt/selectQ001_6'),
//            dataType: 'json', //返回格式为json      
//            async: true, //请求是否异步,默认为异步,这也是ajax重要特性       
//            data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名       
//            type: 'post', //请求方式 get 或者post ,       
