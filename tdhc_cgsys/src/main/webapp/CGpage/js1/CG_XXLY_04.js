                ...
                },
                {
                    ...
                    buttonOptions: {
                        ...
                        onClick: function() {
                            ...
                            if (rowsData.cCheckstate == 'xcc003' || rowsData.cCheckstate == 'xcc002') {
                            ...
                            if (M1S11U_serDel_mark != 'M1S11U_success') {} else {
                                ...
                                result.done(function(dialogResult) {
                                    if (dialogResult) {
                                        ...
                                        $.ajax({
                                            url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_MNHT/M1S11U_XXK2'),
...
//					}
					...
					if (M1S11U_serDel_mark != 'M1S11U_success') {
						...
						result.done(function (dialogResult) {
							if (dialogResult) {
								cake.Ui.LoadPanel.show()
								//../../tdhc_cgsys/CG_MNHT/M1S11U2     ../../tdhc_cgsys/CG_MNHT/M1S11U_XXK2
								$.ajax({
...
//			mode: 'multiple'
	...
	function M1S11_serDel() {
		...
		if (M1S11Q_serDel_mark != 'M1S11Q_success') {
			...
			$.ajax({
				url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_XXLY/M1S11Q'),
				dataType: 'json',   //返回格式为json      
				async: true,//请求是否异步,默认为异步,这也是ajax重要特性       
				data: JSON.stringify(msg),    //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名       
				type: 'post',   //请求方式 get 或者post ,       
...
//				}
				...
				let cRecause = $('#addForm2').dxForm('instance').option('formData').cRecause;
				 if (cRecause == "" || cRecause == undefined || cRecause == null) {
