...
$(function() {
    ...
    //                             $.ajax({
    //                                 url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_MNHT/M1S11U_XXK'),
    ...
    //                 let url = "http://10.107.3.254:8008/WebReport/ReportServer?reportlet=XXK_021.cpt&cConnum=" + cConnum;
    ...
    function M1S21_serDel() {
        ...
        let endTime = msg.data.cgMnhtM1s1[0].cCreatetime_end;
        msg.data.tpCgTime.endTime = new Date(endTime.getFullYear(), endTime.getMonth(), endTime.getDate(), 23, 59, 59, 999);
        M1S11Q_serDel_Judgment();
        //form2中的数据
        ...
        if (msg.data.tpCgcontractmt.length == 0) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    let select;
                    select = data.data.tbDocumeformson
                    tabledataS1.splice(0, tabledataS2.length);
                    select.forEach(item => tabledataS2.push(item));
                    $('#dataGridS2').dxDataGrid('instance').deselectAll()
                    $('#dataGridS2').dxDataGrid('instance').refresh()
                    DevExpress.ui.notify(data.msg, 'success', 3000);
