...
let USER = [];
/*let url = Cake.Piper.getAuthUrl('../../tdhc_cgsys/tj/searchTpCgorderstTJ');*/
let S1S21addIns;
let C0041addIns;
let M1S13addIns;
let C00B1addIns;
...
//     // console.log(n2,n1)
//     var parameter = decodeURI(loc.substr(n2 + 1, n1 - n2));//截取从?号后面的内容
//     var url = parameter.split('=')// url[0]是竖行的数据，url[1]是横行的数据
//     console.log(url)
//     let url3 = url[2]
//     url3 = url3.split('-')
//     // console.log(url3)
...
//             }
//         };
//          let urll = '../../tdhc_cgsys/CG_q002mt/selectQ001_71?'+url[0]+'='+url[1]
////         let urll =Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_q002mt/selectQ001_7')
...
//            }
    ...
    searchdataFormM1.getEditor('startTime').option('value','2020-01-01');   
...
//                          }
//                      };
//                      msg.data.cgQ001M1s14 = searchdataFormM1.option('formData');
//                      //开始时间
//                      let startTime = msg.data.cgQ001M1s14.startTime;
////                      msg.data.cgQ001M1s13.startTime = new Date(startTime.getFullYear(), startTime.getMonth(), startTime.getDate(), 0, 0, 0, 0);
//                      msg.data.cgQ001M1s14.startTime = searchdataFormM1.getEditor('startTime').option('value');
//                      // 结束时间
//                      let endTime = msg.data.cgQ001M1s14.endTime;
//                      msg.data.cgQ001M1s14.endTime = new Date(endTime.getFullYear(), endTime.getMonth(), endTime.getDate(), 23, 59, 59, 999);
//                      msg.data.cgQ001M1s14 = [searchdataFormM1.option('formData')];
//                      console.log(JSON.stringify(msg))
//                  $.ajax({
//                      url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_q002mt/selectQ001_7'),
//                      dataType: 'json', //返回格式为json   
//                      async: true, //请求是否异步,默认为异步,这也是ajax重要特性          
//                      data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名         
//                      type: 'post', //请求方式 get 或者post ,         
...
//                $("#products-data-source3").dxTagBox('instance').option("value", moren)
...
//        	return;
//        }
        msg.data.tpCgordermt3 = searchdataFormM1.option('formData');
        //开始时间
        let startTime = msg.data.tpCgordermt3.startTime;
//        msg.data.cgQ001M1s13.startTime = new Date(startTime.getFullYear(), startTime.getMonth(), startTime.getDate(), 0, 0, 0, 0);
        msg.data.startTime = searchdataFormM1.getEditor('startTime').option('value');
        // 结束时间
        let endTime = msg.data.tpCgordermt3.endTime;
        msg.data.endTime = new Date(endTime.getFullYear(), endTime.getMonth(), endTime.getDate(), 23, 59, 59, 999);
//        msg.data.cgQ001M1s14 = [searchdataFormM1.option('formData')];
//        homePage(url,msg,'dataGridS2',tabledataS2);
//        console.log(JSON.stringify(msg))
        $.ajax({
            url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/tj/searchTpCgorderstTJ'),
            dataType: 'json', //返回格式为json      
            async: true, //请求是否异步,默认为异步,这也是ajax重要特性       
            data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名       
            type: 'post', //请求方式 get 或者post ,       
...
//    	        msg.data.cgQ001M1s13.startTime = new Date(startTime.getFullYear(), startTime.getMonth(), startTime.getDate(), 0, 0, 0, 0);
    	        ...
    	        msg.data.cgQ001M1s14 = [searchdataFormM1.option('formData')];
//    	        console.log(msg)
    	        let urll = Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_q002mt/selectQ001_10');
    	        homePage(urll,msg,'dataGridS2',tabledataS2);
    }
...
//          };
//    	  $.ajax({
//              url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_q002mt/selectQ001_9'),
//              dataType: 'json', //返回格式为json   
//              async: true, //请求是否异步,默认为异步,这也是ajax重要特性          
//              data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名         
//              type: 'post', //请求方式 get 或者post ,         
