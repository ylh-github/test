            	...
            	buttonOptions: {
            		...
            		onClick: function() {
            			...
            			msg = {
            		        ...
            		        //////console.log(msg)           		        
            				         $.ajax({
							            url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_H001/M1S11Q1'),
							            dataType: 'json', //返回格式为json      
							            async: true, //请求是否异步,默认为异步,这也是ajax重要特性       
							            data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名       
							            type: 'post', //请求方式 get 或者post ,       
...
//                        }
            			...
            			// 调用模态框函数    
            			S1S21CK()
                        S1S21addIns.getEditor('cOuttime').option('value',new Date());
            ...
            },
            {
                ...
                buttonOptions: {
                    ...
                    onClick: function() {
                        ...
                        if (selectedRowKeyss1.length < 1) {
                            // Cake.Ui.Toast.showInfo('请选择一条要修改的数据。')     
                            DevExpress.ui.notify('请选择要退票的数据。', 'info', 3000);
                            return;
                        }
                        if (selectedRowKeyss1.length > 1) {
                            // Cake.Ui.Toast.showInfo('一次只能选择一条要修改的数据。')    
                            DevExpress.ui.notify('一次只能选择一条要退票的数据。', 'info', 3000);
                            return;
                        }
                    	msg = {
                                ver: '53cc62f6122a436083ec083eed1dc03d',
	                    ...
	                    var result = DevExpress.ui.dialog.confirm("您确定要退票吗?", "退票确认");
                        result.done(function (dialogResult) {
                            if (dialogResult) {
                                    cake.Ui.LoadPanel.show()
	                    
				                    $.ajax({
				                        url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/moneyinvbook/refundFAPIAO'),
				                        dataType: 'json', //返回格式为json           
				                        async: true, //请求是否异步,默认为异步,这也是ajax重要特性          
				                        data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名         
				                        type: 'post', //请求方式 get 或者post ,         
...
//                }
                msg = {
                    ...
                    let grid = $('#dataGridS1').dxDataGrid('instance');
                    let rowsData = grid.getSelectedRowsData()
                    msg.data = M1S11addIns.option('formData');
                    let gettime = msg.data.cGettime;
...
//                console.log(JSON.stringify(msg))
    ...
    function M1S11_F4() {
        ...
        var result = DevExpress.ui.dialog.confirm("您确定要一键付清吗?", "付清确认");
        	result.done(function (dialogResult) {
        		if (dialogResult) {
        			cake.Ui.LoadPanel.show()
