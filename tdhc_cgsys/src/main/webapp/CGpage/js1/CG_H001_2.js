                ...
                buttonOptions: {
                    ...
                    onClick: function() {
                        ...
                        if (selectedRowKeys.length < 1) {
                            ...
                            DevExpress.ui.notify('请选择一个合同。', 'info', 3000);
                            return;
                        }
                        if (selectedRowKeys.length > 1) {
    ...
    function M1S11Revo() {
        ...
        $('#addsure').dxButton({
            ...
            onClick: function(e) {
              ...
              let grid = $('#dataGridS1').dxDataGrid('instance');
              let rowsData = grid.getSelectedRowsData();     
                msg.data.conrevocare = M1S11addIns.option('formData');
                msg.data.stList = rowsData;
                //change等于1为添加         
                $.ajax({
                    url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_CONMT/AppRevocation'),
                    dataType: 'json', //返回格式为json           
                    async: true, //请求是否异步,默认为异步,这也是ajax重要特性          
                    data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名         
                    type: 'post', //请求方式 get 或者post ,         
...
//        console.log(uken1)
//        console.log(param)
        var startTime = new Date(param[3])
//        console.log(startTime)
        var endTime = new Date(param[4])
//        console.log(endTime)
//        if(param[0]!=null && param[0]!=undefined){
    	    ...
    	    console.log(JSON.stringify(msg))
//    	   var urll =  '../../tdhc_cgsys/CG_CONMT/selectHtInfo4444?'+url[0]+'='+url[1]
//    	    console.log(urll);
    	    $.ajax({
                url: '../../tdhc_cgsys/CG_H001/selectHtInfo?'+url[0]+'='+url[1],
                dataType: 'json', //返回格式为json   
                async: true, //请求是否异步,默认为异步,这也是ajax重要特性          
                data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名         
                type: 'post', //请求方式 get 或者post ,         
