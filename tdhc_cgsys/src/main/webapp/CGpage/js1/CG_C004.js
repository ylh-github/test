...
$(function() {
    ...
    $('#dataGridS1').dxDataGrid({
        ...
        columns: [{
            ...
            },
            {
                ...
                caption: '类型',
                  calculateDisplayValue: function(rowData) {
                    let matchedItem = wuzileixing.find(item => item.value == rowData.cSubitemid);
                    if (!!matchedItem) {
            ...
            },
            {
                ...
                caption: '是否急需',
                 calculateDisplayValue: function(rowData) {
                    let matchedItem = shifoujixu.find(item => item.value == rowData.cSubitemid);
                    if (!!matchedItem) {
                        return matchedItem.cSubitemdes;
                    }
                    return '';
            ...
            },
            {
                ...
                caption: '采购进度',
                  	calculateDisplayValue: function(rowData) {
                    let matchedItem = caigoujindu.find(item => item.value == rowData.cSubitemid);
                    if (!!matchedItem) {
                        return matchedItem.cSubitemdes;
                    }
                    return '';
            ...
            },
            {
                ...
                caption: '采购部门',
                calculateDisplayValue: function(rowData) {
                    let matchedItem = caigoubumen.find(item => item.value == rowData.cSubitemid);
                    if (!!matchedItem) {
                        return matchedItem.cSubitemdes;
                    }
                    return '';
            ...
            },
            {
                ...
                caption: '采购员',
                 calculateDisplayValue: function(rowData) {
                	let matchedItem = caigouyuan.find(item => item.value == rowData.cSubitemid);
                    if (!!matchedItem) {
                        return matchedItem.cSubitemdes;
                    }
                    return '';
            ...
            },
            {
                ...
                caption: '分配状态',
                 calculateDisplayValue: function(rowData) {
                    let matchedItem = fenpeizhuangtai.find(item => item.value == rowData.cSubitemid);
                    if (!!matchedItem) {
                        return matchedItem.cSubitemdes;
                    }
                    return '';
    ...
    function M1S11_serDel() {
        ...
        msg.data.cgC004M1s1 = searchdataFormM1.option('formData')
        //开始时间
		let startTime =msg.data.cgC004M1s1.startTime;
		msg.data.cgC004M1s1.startTime = new Date(startTime.getFullYear(), startTime.getMonth(), startTime.getDate(), 0, 0, 0, 0);

		// 结束时间
		let endTime = msg.data.cgC004M1s1.endTime;
		msg.data.cgC004M1s1.endTime = new Date(endTime.getFullYear(), endTime.getMonth(), endTime.getDate(), 23, 59, 59, 999);
		msg.data.cgC004M1s1 = [searchdataFormM1.option('formData')];
//		//console.log.log(msg);
...
//                    //console.log.log(result.data);
        ...
        msg.data.cgC004M1s1 = [searchdataFormM1.option('formData')];
        $.ajax({
            url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_C004/M1S11Q'),
