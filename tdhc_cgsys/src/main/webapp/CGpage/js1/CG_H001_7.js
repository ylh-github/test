        ...
        columns: [
            ...
            },
            {
                ...
                calculateDisplayValue: function(rowData) {
                    let matchedItem = cgy.find(item => item.cSubitemid == rowData.cSw01);
           ...
           
            {
            	...
            	caption: '对比结果',
            	calculateDisplayValue: function(rowData) {
            		let matchedItem = dhldb.find(item => item.cSubitemid == rowData.cState);
            		if (matchedItem == null || matchedItem == undefined) {
            			return "";
            		} else {
            			return matchedItem.cSubitemdes;
          ...
          },
          {
              ...
              buttonOptions: {
                  ...
                  onClick: function() {
                        ...
                        addIns.show();
	          			M1S11_F1();
	          			M1S11addIns.getEditor('cGettime').option('value',rowsData.cGettime);
                  }
              }
            },
            {
...
//            		        
            		        msg.data.stList = $('#dataGridS1').dxDataGrid('instance').getSelectedRowKeys();
            		        console.log(JSON.stringify(msg))
            		        $.ajax({
            		            url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_CONST/S1S21DBDC'),
...

            {
                ...
                buttonOptions: {
                    ...
                    onClick: function() {
            			...
            			// 调用模态框函数    
            			S1S21DH()
                        S1S21addIns.getEditor('cDeltime').option('value',new Date());                             
                    }
                }
            },
            {
                ...
                buttonOptions: {
                    ...
                    onClick: function() {
                        ...
                        msg.data.tpCgcontrast = rowsDatas1[0];                        
                        ...
                        result.done(function (dialogResult) {
                            if (dialogResult) {
                                    ...
                                    $.ajax({
                                        url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_CONTRAST/S1S21SD'),
                                        dataType: 'json', //返回格式为json   
                                        async: true, //请求是否异步,默认为异步,这也是ajax重要特性   
                                        data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名 
                                        type: 'post', //请求方式 get 或者post , 
            ...
            },
            {
                ...
                buttonOptions: {
                    ...
                    onClick: function() {
                        ...
                        msg.data.excel = $('#dataGridS2').dxDataGrid('instance').getSelectedRowsData();
                        msg.data.tpCgcontrast = selectedRowKeyss1[0];
                        ...
                        result.done(function (dialogResult) {
                            if (dialogResult) {
                                    ...
                                    $.ajax({
                                        url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_CONTRAST/S1S21QXSD'),
                                        dataType: 'json', //返回格式为json   
                                        async: true, //请求是否异步,默认为异步,这也是ajax重要特性   
                                        data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名 
                                        type: 'post', //请求方式 get 或者post , 
                    ...
                    onClick: function() {
                        ...
                        result.done(function (dialogResult) {
                            if (dialogResult) {
			                    ...
			                    $.ajax({
			                        url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_CONTRAST/S1S51U'),
			                        dataType: 'json', //返回格式为json           
			                        async: true, //请求是否异步,默认为异步,这也是ajax重要特性          
			                        data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名         
			                        type: 'post', //请求方式 get 或者post ,         
                    ...
                    onClick: function() {
                        ...
                        result.done(function (dialogResult) {
                            if (dialogResult) {
                                cake.Ui.LoadPanel.show()
		                        $.ajax({
		                            url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_CONTRAST/deleteByPrimaryKey'),
		                            dataType: 'json', //返回格式为json   
		                            async: true, //请求是否异步,默认为异步,这也是ajax重要特性   
		                            data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名 
		                            type: 'post', //请求方式 get 或者post , 
        ...
        function initLoad17() {
        	...
        	$.ajax({
        		...
        		success: function(result) {
        			...
        			searchdataFormM1.getEditor('cState').option('value', 'dhl01');
        		},
        		error: function() {
        			cake.Ui.LoadPanel.close()
        		}
...
//        msg.data.h002request.endTime = new Date(endTime.getFullYear(), endTime.getMonth(), endTime.getDate(), 23, 59, 59, 999);
        msg.data.tpCgcontrast = searchdataFormM1.option('formData');
        console.log(JSON.stringify(msg))
        $.ajax({
            url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_CONTRAST/M1S1Q'),
            dataType: 'json', //返回格式为json      
            async: true, //请求是否异步,默认为异步,这也是ajax重要特性       
            data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名       
            type: 'post', //请求方式 get 或者post ,       
...
//                    msg.data.gettime = new Date(gettime.getFullYear(), gettime.getMonth(), gettime.getDate(), 0, 0, 0, 0);
                    msg.data.gettime = gettime;
                    msg.data.cSw06 = m1s11addIns.cSw06
                    msg.data.tpCgcontrast = rowsData[0];
                   // console.log(JSON.stringify(msg))
            			        $.ajax({
            			            url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_CONTRAST/YJdaohuo'),
            			            dataType: 'json', //返回格式为json      
            			            async: true, //请求是否异步,默认为异步,这也是ajax重要特性       
            			            data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名       
            			            type: 'post', //请求方式 get 或者post ,       
...
//        msg.data.cgH001S1s2 = $('#dataGridS1').dxDataGrid('instance').getSelectedRowsData();
        msg.data.tpCgcontrast = $('#dataGridS1').dxDataGrid('instance').getSelectedRowsData()[0];
        cake.Ui.LoadPanel.show();
        //console.log(msg.data.cgH001S1s2)
        if(msg.data.tpCgcontrast.length==0){
        	DevExpress.ui.notify('请选择需要查询数据', 'info', 3000);
        }else{
        	$.ajax({
                url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_CONTRAST/S1S21Q'),
                dataType: 'json', //返回格式为json  
                async: true, //请求是否异步，默认为异步，这也是ajax重要特性 
                data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名
                type: 'post', //请求方式 get 或者post ,
    ...
    function S1S51_SSRT() {
        ...
        if(msg.data.length==0){
        	...
        	$.ajax({
                url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_CONTRAST/selectList'),
                dataType: 'json', //返回格式为json  
                async: true, //请求是否异步，默认为异步，这也是ajax重要特性 
                data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名
                type: 'post', //请求方式 get 或者post ,
...
//                }
            	msg = {
                    ...
                    console.log(deltime)
                    msg.data.gettime = new Date(deltime.getFullYear(), deltime.getMonth(), deltime.getDate(), 0, 0, 0, 0);
                    msg.data.cSw06 = s1s21addIns.cSw06;
                    msg.data.tpCgcontrast = rowsData[0];
                    msg.data.excel = $('#dataGridS2').dxDataGrid('instance').getSelectedRowsData();
                   // console.log(JSON.stringify(msg))
                    $.ajax({
                        url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_CONTRAST/S1S21DH'),
                        dataType: 'json', //返回格式为json   
                        async: true, //请求是否异步,默认为异步,这也是ajax重要特性   
                        data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名 
                        type: 'post', //请求方式 get 或者post , 
      		...
      		},
      		{
      			...
      			buttonOptions: {
      				...
      				onClick: function() {
      					addIns1.show();
      					$.ajax({
      						url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_CONTRAST/Dbdaoru'),
      						type: 'POST',
      						dataType: 'json',
...
function importf(obj) {
    ...
    */
    ...
    reader.onload = function(e) {
        ...
        tables = tables.replace(/最终到货入库量/g,'cFinalarrnum');
...
//        		cManor:null
        ...
        let grid1 = $('#dataGridS1').dxDataGrid('instance');
        let selectedRowKeys = grid1.getSelectedRowKeys()[0]
        updata.data.TpCgcontrast=selectedRowKeys;
        updata.data.contrastlist = {}
        updata.data.contrastlist = JSON.parse(tables);
