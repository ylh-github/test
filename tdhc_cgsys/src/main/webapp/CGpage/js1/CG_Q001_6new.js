...
//                      let rowsData = grid1.getSelectedRowsData()
                      ...
                      msg.data.q001M1s1Qrequest = $('#dataGridS2').dxDataGrid('instance').getSelectedRowsData();
//                      console.log(msg);
                      $.ajax({
                          url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_Q001/GYBCH'),
...
//                      let rowsData = grid1.getSelectedRowsData()
                      ...
                      msg.data.q001M1s1Qrequest = $('#dataGridS2').dxDataGrid('instance').getSelectedRowsData();
                      $.ajax({
                          url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_Q001/CXCH'),
                          dataType: 'json', //返回格式为json   
                          async: true, //请求是否异步,默认为异步,这也是ajax重要特性   
                          data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名 
                          type: 'post', //请求方式 get 或者post , 
...
//                                  }
                          ...
                          //开始时间
                          let startTime = msg.data.cgQ001M1s13[0].startTime;
//                          msg.data.cgQ001M1s13.startTime = new Date(startTime.getFullYear(), startTime.getMonth(), startTime.getDate(), 0, 0, 0, 0);
                          msg.data.cgQ001M1s13.startTime = searchdataFormM1.getEditor('startTime').option('value');
                          // 结束时间
                          let endTime = msg.data.cgQ001M1s13[0].endTime;
...
//                          msg.data.cgQ001M1s14 = [searchdataFormM1.option('formData')];
                      ...
                      $.ajax({
                          url: Cake.Piper.getAuthUrl('../../tdhc_cgsys/CG_q002mt/selectQ001_8'),
                          dataType: 'json', //返回格式为json   
                          async: true, //请求是否异步,默认为异步,这也是ajax重要特性          
                          data: JSON.stringify(msg), //下拉框的参数值  queryCondition 上面有定义 cStlCla 后台给的固定的名         
                          type: 'post', //请求方式 get 或者post ,         
...
//                searchdataFormM1.getEditor('cManor').option('value', get_user());
    ...
    function M1S11_serDel() {
        ...
        msg.data.cgQ001M1s13 = searchdataFormM1.option('formData');
        console.log(msg.data.cgQ001M1s13);
        //开始时间
...
//        console.log(JSON.stringify(msg))
        $.ajax({
            ...
            success: function(data) {
                ...
                if (select == null) {
                	tabledataS2.splice(0, tabledataS2.length);
                    $('#dataGridS1').dxDataGrid('instance').option('dataSource', tabledataS2)
                    tabledataS2.splice(0, tabledataS2.length);
                    $('#dataGridS2').dxDataGrid('instance').option('dataSource', tabledataS2)
...
//            },
    ...
    function createStaticText2(data) {
        ...
        return $('<span>').text(data.component.option('formData')[data.dataField]).css({ 'color': 'red', 'font-weight': 'bold' });
