...
//		}else 
		...
		nomoney = new BigDecimal(conmt.get(0).getcSw02());//合同未付金额
		...
		if(nomoney.compareTo(BigDecimal.ZERO) == 0) {
			conmt.get(0).setcSignstep("1");
		}
...
//		if(conmt.get(0).getcDr() == null) {
//			List<TpCgcontractmtt> cgcontractmtt = tpCgcontractmttMapper.selectBycConnum(conmt.get(0).getcConnum());
//			if(cgcontractmtt.size() > 0) {
//				System.out.println(cgcontractmtt.size() + " bbbbbbbbbbbbbbbbbb");
//				List<TpCgmoneybook> tpCgmoneybook = tpCgmoneybookMapper.selectPaytimeMin(conmt.get(0).getcConnum());			
//				Date date = null;
//				if(tpCgmoneybook.size() > 0) {
//					System.out.println(tpCgmoneybook.size() + " ccccccccccccccccccccccccc");
//					date = tpCgmoneybook.get(0).getcPaytime();
//				}
//				if("qx01".equals(cgcontractmtt.get(0).getcDelivery()) && cgcontractmtt.get(0).getcDeliveryday() != null && date != null) {
////					cgcontractmtt.get(0).setcDeliverytime(date);
////					cgcontractmtt.get(0).setcDr(ContractUtil.dateUnit(Integer.parseInt(cgcontractmtt.get(0).getcDeliverydat()), date));
//					conmt.get(0).setcDr(ContractUtil.dateUnit(Integer.parseInt(cgcontractmtt.get(0).getcDeliveryday()), date));
//					System.out.println(conmt.get(0).getcDr() + " aaaaaaaaaaaaaaaaaaaaaa");
//				}
//			}
//		}
	...
	public ResponseObject<EmptyTag, EmptyData> updateFUKUAN(RequestObject<EmptyTag, MoneyBook> requestObject) {
		...
		BigDecimal bookm = new BigDecimal(book0.getcClearmoney());//修改前申请金额
		int row = 0;
		...
		money = new BigDecimal(conmt.get(0).getcSw01());//获取修改后的已付金额
		nomoney = new BigDecimal(conmt.get(0).getcSw02());//获取修改后合同未付金额
		//判断已付金额是否为0  为0时 合同未付款
		if(money.compareTo(BigDecimal.ZERO) == 0) {
			conmt.get(0).setcSignstep("0");
		}else {
	...
	@Override
	public ResponseObject<EmptyTag, List<TpCgmoneybook>> derivedFUKUAN(RequestObject<EmptyTag, MoneyBook> requestObject) {
		ResponseObjectBuilder builder = ResponseObjectBuilder.create();
		List<TpCgmoneybook> moneybooklist = tpCgmoneybookMapper.selectByCreatetime(requestObject.getData().getStartTime(), requestObject.getData().getEndTime());		
		return builder.data(moneybooklist).errcode(Errcode.Success).build();
	}
