package code_fb_cg.request;

/**
 * 模拟合同物资
 * @author lhb
 * @时间：2019年2月18日上午10:34:09
 */
public class TpCgorderRequest {
	//主键
	private String cId;
	/** 物资名称 **/
	private String cGoodsname;
	/** 请购单号 **/
	private String cOrdernum;
	/** 采购员 **/
	private String cManor;
	/** 报关物资名称 **/
	private String cCustoms;
	/** 规格型号 **/
	private String cSpec;
	/** 报关物资规格型号 **/
	private String cCuspec;
	/** 单位 **/
	private String cUnit;
	/** 订货数量 **/
	private String cNum;
	/** 含税单价 **/
	private String cPrice;
	/** 含税总价:单价*订货数量 **/
	private String cSumprice;
	/** 备注 **/
	private String cRemark;
	
	private String cRemarkc;
	/** 档案号 **/
	private String cNo;
	/** 请购单主键 **/
	private String cMtid;
	//订购数量
	private String cGoodsnum;
	//报关物资名称
	private String cSw08;
	private String cSw02;
	private String cOrid;
	private String cSw03;
	private String cSw05;
	private String cSw04;
	private String cConnum;
	private String cConline;
	
	//报关单位
	private String cUnitspec;
	//method
	public String getcId() {
		return cId;
	}
	
	
	public String getcUnitspec() {
		return cUnitspec;
	}


	public void setcUnitspec(String cUnitspec) {
		this.cUnitspec = cUnitspec;
	}


	public String getcSw02() {
		return cSw02;
	}


	public void setcSw02(String cSw02) {
		this.cSw02 = cSw02;
	}


	public String getcOrid() {
		return cOrid;
	}


	public void setcOrid(String cOrid) {
		this.cOrid = cOrid;
	}


	public String getcSw03() {
		return cSw03;
	}


	public void setcSw03(String cSw03) {
		this.cSw03 = cSw03;
	}


	public String getcSw05() {
		return cSw05;
	}


	public void setcSw05(String cSw05) {
		this.cSw05 = cSw05;
	}


	public String getcSw04() {
		return cSw04;
	}


	public void setcSw04(String cSw04) {
		this.cSw04 = cSw04;
	}


	public String getcGoodsnum() {
		return cGoodsnum;
	}


	public void setcGoodsnum(String cGoodsnum) {
		this.cGoodsnum = cGoodsnum;
	}


	public String getcSw08() {
		return cSw08;
	}


	public void setcSw08(String cSw08) {
		this.cSw08 = cSw08;
	}


	public String getcRemarkc() {
		return cRemarkc;
	}


	public void setcRemarkc(String cRemarkc) {
		this.cRemarkc = cRemarkc;
	}


	public String getcMtid() {
		return cMtid;
	}


	public void setcMtid(String cMtid) {
		this.cMtid = cMtid;
	}


	public String getcCuspec() {
		return cCuspec;
	}
	public void setcCuspec(String cCuspec) {
		this.cCuspec = cCuspec;
	}
	public String getcNo() {
		return cNo;
	}
	public void setcNo(String cNo) {
		this.cNo = cNo;
	}
	public String getcManor() {
		return cManor;
	}
	public void setcManor(String cManor) {
		this.cManor = cManor;
	}
	public void setcId(String cId) {
		this.cId = cId;
	}
	public String getcGoodsname() {
		return cGoodsname;
	}
	public void setcGoodsname(String cGoodsname) {
		this.cGoodsname = cGoodsname;
	}
	public String getcOrdernum() {
		return cOrdernum;
	}
	public void setcOrdernum(String cOrdernum) {
		this.cOrdernum = cOrdernum;
	}
	public String getcCustoms() {
		return cCustoms;
	}
	public void setcCustoms(String cCustoms) {
		this.cCustoms = cCustoms;
	}
	public String getcSpec() {
		return cSpec;
	}
	public void setcSpec(String cSpec) {
		this.cSpec = cSpec;
	}
	public String getcUnit() {
		return cUnit;
	}
	public void setcUnit(String cUnit) {
		this.cUnit = cUnit;
	}
	
	public String getcNum() {
		return cNum;
	}
	public void setcNum(String cNum) {
		this.cNum = cNum;
	}


	public String getcPrice() {
		return cPrice;
	}
	public void setcPrice(String cPrice) {
		this.cPrice = cPrice;
	}
	public String getcSumprice() {
		return cSumprice;
	}
	public void setcSumprice(String cSumprice) {
		this.cSumprice = cSumprice;
	}
	public String getcRemark() {
		return cRemark;
	}
	public void setcRemark(String cRemark) {
		this.cRemark = cRemark;
	}
	

	public String getcConnum() {
		return cConnum;
	}


	public void setcConnum(String cConnum) {
		this.cConnum = cConnum;
	}


	public String getcConline() {
		return cConline;
	}


	public void setcConline(String cConline) {
		this.cConline = cConline;
	}
	
}
